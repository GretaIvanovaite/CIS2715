...
@section('page-title', 'Question website - add question options')
@section ('h1-title', 'Multi-choice or grid question options')
...
@section('main-content')
   <section id='create_question' class="flex flex-col w-full lg:w-1/2 mx-auto p-8 lg:p-10 rounded-2xl shadow-xl bg-white">
        ...
        <form class="flex flex-col" id="question_form" method="POST" action="{{ route('options.store', $questionnaire->id) }}">
            @csrf
            ...
            <input type="checkbox" name="again" id="again" value="Y">
            <label for="again" class="inline-block text-sm md:text-base align-middle self-center"></label>
