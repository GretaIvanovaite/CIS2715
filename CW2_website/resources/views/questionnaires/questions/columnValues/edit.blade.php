...
@section('page-title', 'Question website - update column values')
@section ('h1-title', 'Update grid question column values')
...
@section('main-content')
   ...
   <section id='create_question' class="flex flex-col w-full lg:w-1/2 mx-auto p-8 lg:p-10 rounded-2xl shadow-xl bg-white">
        ...
        <form class="flex flex-col" id="column_form" method="POST" action="{{ route('columns.update', [$questionnaire->id, $question->id, $column->id]) }}">
            ...
            @enderror
            <input type="text" name="text" id="text" class="pl-4 mb-2 bg-gray-50 text-gray-600 border focus:border-transparent border-gray-300 text-sm md:text-base rounded-lg ring-3 ring-transparent focus:ring-1 focus:outline-hidden focus:ring-gray-400 block w-full p-2.5 rounded-l-lg py-3 px-4" value="{{ old('text') !== null ? old('text') : $column->text }}" required>
