   ...
   <section id='edit_questionnaire' class="flex flex-col w-full lg:w-1/2 mx-auto p-8 lg:p-10 rounded-2xl shadow-xl bg-white">
        ...
        <form class="flex flex-col" id="questionnaire_form" method="POST" action="{{ route('questionnaires.update', $questionnaire->id) }}">
            ...
            @error('consent')
                <span class="text-red-500 text-xs md:text-sm">{{ $message }}</span>
            @enderror
            <input type="text" name="consent" id="consent" class="pl-4 mb-2 bg-gray-50 text-gray-600 border focus:border-transparent border-gray-300 text-sm md:text-base rounded-lg ring-3 ring-transparent focus:ring-1 focus:outline-hidden focus:ring-gray-400 block w-full p-2.5 rounded-l-lg py-3 px-4" value="{{ old('consent') !== null ? old('consent') : $questionnaire->consent }}" required>
            ...
            @error('consent')
                <span class="text-red-500 text-xs md:text-sm">{{ $message }}</span>
            @enderror
            <textarea rows=5 name="consent" id="consent" class="resize pl-4 mb-2 bg-gray-50 text-gray-600 border focus:border-transparent border-gray-300 text-sm md:text-base rounded-lg ring-3 ring-transparent focus:ring-1 focus:outline-hidden focus:ring-gray-400 block w-full p-2.5 rounded-l-lg py-3 px-4">{{ old('consent') !== null ? old('consent') : $questionnaire->consent }}</textarea>
