...
@section('main-content')
   <section id='create_questionnaire' class="flex flex-col w-full lg:w-1/2 mx-auto p-8 lg:p-10 rounded-2xl shadow-xl bg-white">
        <h2 class="text-lg md:text-xl font-bold pb-8 my-auto">Please enter your questionnaire details</h2>
        <form class="flex flex-col" id="questionnaire_form" method="POST" action="{{ route('questionnaires.store') }}">
            ...
            <textarea name="description" id="description" class="pl-4 mb-2 bg-gray-50 text-gray-600 border focus:border-transparent border-gray-300 text-sm md:text-base rounded-lg ring-3 ring-transparent focus:ring-1 focus:outline-hidden focus:ring-gray-400 block w-full p-2.5 rounded-l-lg py-3 px-4">
            </textarea>
            <input type="hidden" name="status" id="status" value='In development'>
