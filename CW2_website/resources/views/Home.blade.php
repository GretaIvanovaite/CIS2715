    ...
    <section id='questionnaires' class="grid grid-cols-1 md:grid-cols-3 min-w-auto">
        @forelse ($questionnaires as $questInstance)
            <article class="bg-white shadow-xl flex flex-col p-8 rounded-2xl m-2 min-w-auto">
                ...
                <a class="cursor-pointer bg-brightgreen text-black font-semibold text-base uppercase rounded-lg p-2 hover:bg-darkgreen hover:text-white hover:font-bold active:scale-95 transition-transform transform m-2 self-center min-w-auto max-w-9/10" href="{{ route('questionnaires.show', $questInstance->id) }}">Take questionnaire!</a>
            </article>
        @empty
