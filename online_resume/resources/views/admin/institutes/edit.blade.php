...
<body class="flex flex-col h-screen justify-between">
    ...
    <main class="max-w-3x1 text-xl text-black mb-auto p-20">
        <h2 class="p-6 pl-0 text-xl font-bold text-cyan-800">Editing Institute ID: {{ $institute->id }}</h2>
        <!-- Form -->
        <form class="max-w-2x1 mx-auto bg-white p-8 rounded-lg shadow-md space-y-6" action="{{ route('institutes.update', $institute->id) }}" method="POST">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $institute->name) }}" placeholder="e.g. Edge Hill University" required class="w-full px-4 py-3 h-14 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-700 focus:border-blue-700">
                @error('name') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <!-- Street -->
            <div>
                <label for="street" class="block text-gray-700 font-semibold">Street:</label>
                <input type="text" id="street" street="street" value="{{ old('street', $institute->street) }}" placeholder="e.g. St Helens Rd" required class="w-full px-4 py-3 h-14 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-700 focus:border-blue-700">
                @error('street') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <!-- Town -->
            <div>
                <label for="town" class="block text-gray-700 font-semibold">Town:</label>
                <input type="text" id="town" town="town" value="{{ old('town', $institute->town) }}" placeholder="Ormskirk" required class="w-full px-4 py-3 h-14 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-700 focus:border-blue-700">
            ...
            <div>
                ...
                <input type="text" id="postcode" postcode="postcode" value="{{ old('postcode', $institute->postcode) }}" placeholder="e.g. L39 4QP" required class="w-full px-4 py-3 h-14 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-700 focus:border-blue-700">
